        ...
        if req.method == "tools/call" {
            ...
            let result = match tool_name {
                "store_memory" => {
                    ...
                    let tenant_id = tool_args.get("tenant_id").and_then(|v| v.as_str()).unwrap_or("default");
                    let fact_text = tool_args.get("fact_text").and_then(|v| v.as_str()).unwrap_or("");
                    let tags: Vec<String> = tool_args.get("tags")
                        .and_then(|v| v.as_array())
                        .map(|arr| arr.iter().filter_map(|v| v.as_str().map(|s| s.to_string())).collect())
                        .unwrap_or_default();
                    
                    match self.app.store_explicit_fact(tenant_id, fact_text, &tags).await {
                ...
                "push_dialogue" => {
                    ...
                    let tenant_id = tool_args.get("tenant_id").and_then(|v| v.as_str()).unwrap_or("default");
                    let session_id = tool_args.get("session_id").and_then(|v| v.as_str()).unwrap_or("default");
                    let new_message = tool_args.get("new_message").and_then(|v| v.as_str()).unwrap_or("");
                    
                    match self.app.push_dialogue(tenant_id, session_id, new_message).await {
                        Ok(context_window) => {
                            let json_ctx = serde_json::to_string(&context_window).unwrap_or("{}".into());
                ...
                "delete_tenant" => {
                    ...
                    match self.app.delete_tenant(tenant_id).await {
                        Ok(_) => McpToolResult::ok(&format!("Successfully deleted all data for tenant {}", tenant_id)),
