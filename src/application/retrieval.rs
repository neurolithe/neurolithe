...
impl RetrievalService {
    ...
    pub async fn query(&self, tenant_id: &TenantId, raw_query: &str, time_filter: &TimeFilter) -> Result<Vec<MemoryResult>> {
        ...
        let results = self.memory_repo.query_with_graph(raw_query, &embedding, tenant_id, time_filter, 10)?;
        
        Ok(results)
    }

    /// Legacy: simple hybrid search without graph traversal (for backward compatibility)
    pub async fn query_simple(&self, tenant_id: &TenantId, raw_query: &str) -> Result<Vec<MemoryNode>> {
        let embedding = self.llm_client.embed_text(raw_query).await?;
        let results = self.memory_repo.hybrid_search(raw_query, &embedding, tenant_id, 10)?;
        Ok(results)
    }
}
